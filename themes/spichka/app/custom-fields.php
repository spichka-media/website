    ...
    function () {
        Container::make('theme_options', __('Настройки темы'))->add_fields([
            ...
            Field::make('complex', 'theme_socials', 'Социальные сети')
                ...
                ->add_fields([Field::make('text', 'theme_social_link', 'Ссылка'), Field::make('text', 'theme_social_icon', 'Иконка')]),
        ...
        Container::make('post_meta', __('Настройки поста'))
            ...
            ->add_tab(__('Комментарии'), [Field::make('checkbox', 'post_show_comments', 'Отображать комментарии')->set_option_value('yes'), Field::make('text', 'post_telegram_post_id', __('Номер поста в Telegram'))]);
        ...
        Container::make('post_meta', __('Настройки главной страницы'))
            ...
            ->add_tab(__('Программные статьи'), [
                ...
                Field::make('association', 'front_program_articles', __('Статьи'))->set_types([
                    [
                        'type' => 'post',
                        'post_type' => 'post',
                    ],
            ...
            ->add_tab(__('Свежие статьи'), [Field::make('text', 'front_recent_articles_header', __('Заголовок'))->set_default_value('Свежие статьи')])
            ->add_tab(__('Рубрики'), [Field::make('text', 'front_article_categories_header', __('Заголовок'))->set_default_value('Рубрики')])
            ->add_tab(__('Присоеденяйся'), [
                ...
                Field::make('complex', 'front_connect_blocks', 'Блоки')
                    ->set_layout('tabbed-horizontal')
                    ->add_fields([Field::make('text', 'front_connect_blocks_block_header', 'Заголовок'), Field::make('text', 'front_connect_blocks_block_description', 'Описание'), Field::make('text', 'front_connect_blocks_block_button_text', 'Текст на кнопке'), Field::make('text', 'front_connect_blocks_block_form', 'Форма')]),
            ])
            ->add_tab(__('Донать'), [Field::make('text', 'front_donate_header', __('Заголовок'))->set_default_value('Помогай'), Field::make('text', 'front_donate_description', __('Описание'))->set_default_value('Мы работаем над проектом в свободное время и на энтузиазме. Чтобы творить, нам нужно тратить деньги на сайт, покупать оборудование для видео и подкастов, снимать студии. Спасибо, что присылаете нам деньги — так нам реже приходится тратить свои.'), Field::make('text', 'front_donate_button_text', __('Текст на кнопке'))->set_default_value('Задонатить'), Field::make('image', 'front_donate_image', __('Изображение'))]);
    },
    100,
);
